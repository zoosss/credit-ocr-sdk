# 拼图配征信OCR识别与征信分析 API 接口文档

> 全国征信OCR识别准确率领先 · 支持私有化本地部署
>
...
除 SaaS API 接入外，拼图配还支持 **私有化本地部署** 方案，所有征信OCR识别和征信分析处理在客户内网完成，数据不出内网，满足银行级安全合规要求。

如需获取 API Token、私有化部署方案或技术支持，请联系我们：
